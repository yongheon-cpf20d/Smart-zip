...
const regulations: {
...
}[] = [
  {
    ...
    details: [
      "주담대 LTV 40% 제한, 다주택자 주담대 실질 불허 수준으로 규제",
      "청약 가점제 비율 대폭 강화 (85㎡ 초과 100% 가점제, 이하도 추첨 비율 최소화)",
      "재건축·재개발 조합원 지위 양도 원칙적 금지",
      "주택 취득 시 자금조달계획서 및 증빙자료 제출 의무화",
  ...
  },
  {
    title: "조정대상지역",
    color: "orange",
    summary: "투기과열지구보다는 완화됐지만 깐깐한 대출·세제 규제가 적용됩니다.",
    details: [
      "주담대 LTV 40% 제한 (수도권·규제지역 공통)",
      "1세대 1주택 양도세 비과세 요건에 '2년 실거주' 의무 추가 (취득 당시 조정대상지역)",
  ...
  },
  {
    title: "토지거래허가구역",
    color: "amber",
    summary: "토지·건물 거래 시 관할 지자체장의 허가가 필수인 지역입니다.",
    details: [
      "전세를 끼고 매수하는 갭투자 원천 차단 (전세 상태 취득 허가 불가)",
      "주택 취득 후 본인이 2년 이상 직접 실거주 의무 부과",
      "상업·업무용 토지는 허가 기간 내 직접 이용 의무 부과",
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans">
      <div className="max-w-5xl mx-auto px-4 py-4 space-y-5">
        ...
        {/* 2026 가계부채 관리방안 */}
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800/50 rounded-2xl p-5">
          <h2 className="text-sm font-bold text-red-700 dark:text-red-400 mb-2 flex items-center gap-1.5">
            🚨 2026년 가계부채 관리방안 특별 주의보
          </h2>
          <ul className="space-y-2 text-xs text-red-600 dark:text-red-300/80 leading-relaxed">
            <li className="flex items-start gap-1.5">
              <span className="mt-0.5 shrink-0">•</span>
              <span><strong>수도권 다주택자 만기연장 불가:</strong> 다주택자 및 임대사업자의 수도권 아파트 주담대는 원칙적으로 만기연장이 불허됩니다. 목돈 일시 상환이 필요하므로 자금 계획에 주의가 필요합니다.</span>
            </li>
            <li className="flex items-start gap-1.5">
              <span className="mt-0.5 shrink-0">•</span>
              <span><strong>가계대출 총량 통제:</strong> 은행권 가계대출 증가율이 엄격히 제한되어 있어, 하반기로 갈수록 대출 한도가 조기 소진될 가능성이 높습니다. 자금 조달 계획을 보수적으로 세우시길 권장합니다.</span>
            </li>
            <li className="flex items-start gap-1.5">
              <span className="mt-0.5 shrink-0">•</span>
              <span><strong>갭투자 정조준:</strong> 실거주하지 않는 주택 보유자 및 사업자대출 우회 이용자에 대한 점검과 대출 회수가 진행 중입니다. 과거 내역 포함 전수 점검 대상임을 유의하세요.</span>
            </li>
          </ul>
        </div>

        <p className="text-[10px] text-slate-400 dark:text-slate-500 pt-2 leading-relaxed">
          출처: 금융위원회 「주택시장 안정화 대책」(2025.10.15), 2026년 가계부채 관리방안, 지방세법 제13조의2.
          스트레스 DSR 가산금리는 금융위원회 정기 고시 기준이며 시장 상황에 따라 변경될 수 있습니다.
          본 정보는 참고용이며 실제 규제지역 지정 현황 및 세부 기준은 국토교통부·금융위원회 공지를 반드시 확인해주세요.
        </p>
