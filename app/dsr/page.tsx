...
function calcMonthlyPI(
  ...
  if (type === "bullet") return principal * mr;
...
export default function DSRPage() {
  ...
  const existMonthly = calcMonthlyPI(
    Number(existAmount) * 10000, Number(existYears), Number(existRate),
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={existLoanType} onChange={(e) => setExistLoanType(e.target.value)}
            ...
            <div>
              ...
              <select value={existRepayType} onChange={(e) => setExistRepayType(e.target.value as RepayType)}
                className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400">
          ...
          <div>
            ...
            <input type="number" value={existAmount} onChange={(e) => setExistAmount(e.target.value)}
              placeholder="예: 5000" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-400 mb-1 block">대출만기 (년)</label>
              <input type="number" value={existYears} onChange={(e) => setExistYears(e.target.value)}
                placeholder="예: 5" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400" />
            </div>
            <div>
              <label className="text-xs text-slate-400 mb-1 block">대출금리 (%)</label>
              <input type="number" step="0.01" value={existRate} onChange={(e) => setExistRate(e.target.value)}
                placeholder="예: 6.5" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-400" />
            </div>
          </div>
